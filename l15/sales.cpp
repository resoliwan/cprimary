#include "sales.h"
#include <iostream>
using namespace std;

Sales::bad_index::bad_index(int ix, const std::string & s): std::logic_error(s), bi(ix)
{
}



Sales::Sales(int yy): year(yy) {
  for (int i = 0; i < MONTHS; ++i)
    gross[i] = 0;
}

Sales::Sales(int yy, const double * gr, int n): year(yy)
{
  int lim = n < MONTHS ? n : MONTHS;
  int i;
  for (i = 0; i < lim; ++i) 
    gross[i] = gr[i];
  for (; i < MONTHS; ++i)
    gross[i] = 0;
}

double Sales::operator[](int i) const
{
  if (i < 0 || i >= MONTHS)
    throw Sales::bad_index(i, "Index error!");
  return gross[i];
}

double & Sales::operator[](int i)
{
  if (i < 0 || i >= MONTHS)
    throw Sales::bad_index(i, "Index error!");
  return gross[i];
}

LabelSales::nbad_index::nbad_index(const std::string & lb, int ix, const std::string & s)
  : Sales::bad_index(ix, s), lbl(lb)
{
}

LabelSales::LabelSales(const std::string & lb, int yy, const double * gr, int n)
  : Sales(yy, gr, n), label(lb)
{
}

double LabelSales::operator[](int i) const
{
  if (i < 0 || i >= MONTHS)
    throw LabelSales::nbad_index("label", i, "Index error!");
  return Sales::operator[](i);
}

double & LabelSales::operator[](int i)
{
  if (i < 0 || i >= MONTHS)
    throw LabelSales::nbad_index("label", i, "Index error!");
  return Sales::operator[](i);
}
